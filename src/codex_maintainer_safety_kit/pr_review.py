...
def _render_target(target: dict[str, Any], index: int) -> str:
    ...
    purpose = target.get("purpose") or "review target"
    target_type = target.get("type") or "target"
    return f"- {index}. `{target_type}`: {locator} ({purpose})"


def render_pr_review(manifest: dict[str, Any]) -> str:
    ...
    if gate.passed:
        lines.extend(
            [
                "- Review this local report before drafting GitHub comments.",
                "- Treat suggested review comments as drafts; do not auto-post them.",
                "- Confirm tests and CI before merging the pull request.",
            ]
        )
    else:
        lines.extend(
...
def write_pr_review(markdown: str, output: str | Path) -> Path:
    output = Path(output)
